...
class MyDataset(Dataset):
    def __init__(self, file_path, tag, word2idx, debug=False):
...
def collate_fn(data):
    ...
    #labels = make_one_hot(torch.cat(labels, dim=0), 2)
    labels = torch.FloatTensor(list(labels))
    return seqs, seq_lens, labels

def prepare_whole_senti_data(hps, vocab):
    print('preparing whole senti data...')
    dataset = DisDataset(hps.whole_senti_train_data_path, hps.whole_senti_train_label_path, vocab, debug=False)
    
    weights = make_weights_for_balanced_classes(dataset.ls, 2, PosOverNeg=1)
    sampler = WeightedRandomSampler(weights, len(weights))
    
    train_data_loader = DataLoader(dataset,\
                             batch_size=hps.whole_senti_batch_size,\
                             shuffle=False,\
                             collate_fn=collate_fn, drop_last=False, sampler=sampler)
    
    dataset = DisDataset(hps.whole_senti_dev_data_path, hps.whole_senti_dev_label_path, vocab, debug=False)
    weights = make_weights_for_balanced_classes(dataset.ls, 2, PosOverNeg=1)
    sampler = WeightedRandomSampler(weights, len(weights))
...
def prepare_seq2seq_data(hps,vocab):
    print('preparing seq data...')
    irony_dataset = MyDataset(hps.irony_path, 1, vocab, debug=False)
    irony_data_loader = DataLoader(irony_dataset,\
                             batch_size=hps.batch_size,\
                             shuffle=True,\
                             collate_fn=collate_fn, drop_last=True)
    
    non_dataset = MyDataset(hps.non_path, 0, vocab, debug=False)
...
def prepare_train_data(samples):
    pass

def prepare_test_data(hps, vocab):
    print('preparing test data...')
    non_dataset = MyDataset(hps.non_test_path, 0, vocab, debug=False)
    non_data_loader = DataLoader(non_dataset,\
                             batch_size=hps.test_batch_size,\
                             shuffle=False,\
                             collate_fn=collate_fn, drop_last=False)
    
    irony_dataset = MyDataset(hps.irony_test_path, 1, vocab, debug=False)
    irony_data_loader = DataLoader(irony_dataset,\
                             batch_size=hps.test_batch_size,\
                             shuffle=False,\
                             collate_fn=collate_fn, drop_last=False)
...
def prepare_dev_data(hps, vocab):
